...
public class MonsterSwipeController : MonoBehaviour
{
    ...
    private Dictionary<GameObject, bool> movingMonsters = new Dictionary<GameObject, bool>();
        ...
        if (Input.GetMouseButtonDown(0))
        {
            ...
            if (Physics.Raycast(ray, out RaycastHit hit))
            {
                if (hit.collider.CompareTag("Monster"))
                {
                    ...
                    if (movingMonsters.ContainsKey(selectedMonster) && movingMonsters[selectedMonster])
                    {
                        Debug.Log("Monster is already moving. Swipe ignored.");
                        isSwiping = false;
                        return;
                    }
